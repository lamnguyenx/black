...
import os
os.environ['FOLLOW_LAMNGUYENX'] = 'False'
...
import blax
import blax.files
from blax import Feature, TargetVersion
from blax import re_compile_maybe_verbose as compile_pattern
from blax.cache import FileData, get_cache_dir, get_cache_file
from blax.debug import DebugVisitor
from blax.mode import Mode, Preview
from blax.output import color_diff, diff
from blax.parsing import ASTSafetyError
from blax.report import Report
from blax.strings import lines_with_leading_tabs_expanded
...
DEFAULT_EXCLUDE = blax.re_compile_maybe_verbose(blax.const.DEFAULT_EXCLUDES)
DEFAULT_INCLUDE = blax.re_compile_maybe_verbose(blax.const.DEFAULT_INCLUDES)
T = TypeVar("T")
R = TypeVar("R")
...
def cache_dir(exists: bool = True) -> Iterator[Path]:
    with TemporaryDirectory() as workspace:
        ...
        with patch("blax.cache.CACHE_DIR", cache_dir):
            yield cache_dir
...
class FakeContext(click.Context):
    ...
    def __init__(self) -> None:
        ...
        self.command: click.Command = blax.main
...
def invokeBlack(
    ...
    result = runner.invoke(blax.main, args, catch_exceptions=False)
    ...
    msg = '\n'.join([
        ...
        f"😀 stdout:",
        textwrap.indent(f'{result.stdout_bytes.decode()}', ' '*4),

        f"🤬 stderr:",
        textwrap.indent(f'{result.stderr_bytes.decode()}', ' '*4),

        f"🖍️ exception: {result.exception}",
    ])
...
class BlackTestCase(BlackBaseTestCase):
    ...
    def test_empty_ff(self) -> None:
        expected = ""
        tmp_file = Path(blax.dump_to_file())
        try:
            self.assertFalse(ff(tmp_file, write_back=blax.WriteBack.YES))
            actual = tmp_file.read_text(encoding="utf-8")
        finally:
            os.unlink(tmp_file)
        self.assertFormatEqual(expected, actual)

    @patch("blax.dump_to_file", dump_to_stderr)
    ...
    def test_one_empty_line_ff(self) -> None:
        for nl in ["\n", "\r\n"]:
            expected = nl
            tmp_file = Path(blax.dump_to_file(nl))
            if system() == "Windows":
                ...
                # https://github.com/psf/blax/pull/3348
                with open(tmp_file, "wb") as f:
                    f.write(nl.encode("utf-8"))
            try:
                self.assertFalse(
                    ff(tmp_file, mode=mode, write_back=blax.WriteBack.YES)
                )
    ...
    #         PROJECT_ROOT / "src/blax/__init__.py"
    #     )
    ...
    #             "-",
    #             "--fast",
    #             f"--line-length={blax.DEFAULT_LINE_LENGTH}",
    ...
    #         blax.assert_equivalent(source, result.output)
    #         blax.assert_stable(source, result.output, DEFAULT_MODE)

    def test_experimental_string_processing_warns(self) -> None:
        self.assertWarns(
            blax.mode.Deprecated, blax.Mode, experimental_string_processing=True
        )
    ...
    def test_piping_diff(self) -> None:
        ...
        result = BlackRunner().invoke(
            blax.main, args, input=BytesIO(source.encode("utf-8"))
        )
        self.assertEqual(result.exit_code, 0)
    ...
    def test_pep_572_version_detection(self) -> None:
        ...
        root = blax.lib2to3_parse(source)
        features = blax.get_features_used(root)
        self.assertIn(blax.Feature.ASSIGNMENT_EXPRESSIONS, features)
        versions = blax.detect_target_versions(root)
        self.assertIn(blax.TargetVersion.PY38, versions)
    ...
    def test_pep_695_version_detection(self) -> None:
        for file in ("type_aliases", "type_params"):
            ...
            root = blax.lib2to3_parse(source)
            features = blax.get_features_used(root)
            self.assertIn(blax.Feature.TYPE_PARAMS, features)
            versions = blax.detect_target_versions(root)
            self.assertIn(blax.TargetVersion.PY312, versions)

    def test_expression_ff(self) -> None:
        source, expected = read_data("cases", "expression.py")
        tmp_file = Path(blax.dump_to_file(source))
        try:
            self.assertTrue(ff(tmp_file, write_back=blax.WriteBack.YES))
            actual = tmp_file.read_text(encoding="utf-8")
        finally:
            os.unlink(tmp_file)
        self.assertFormatEqual(expected, actual)
        with patch("blax.dump_to_file", dump_to_stderr):
            blax.assert_equivalent(source, actual)
            blax.assert_stable(source, actual, DEFAULT_MODE)
    ...
    def test_expression_diff(self) -> None:
        ...
        try:
            result = BlackRunner().invoke(
                blax.main, ["--diff", str(tmp_file), f"--config={EMPTY_CONFIG}"]
            )
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
    ...
    def test_detect_pos_only_arguments(self) -> None:
        ...
        root = blax.lib2to3_parse(source)
        features = blax.get_features_used(root)
        self.assertIn(blax.Feature.POS_ONLY_ARGUMENTS, features)
        versions = blax.detect_target_versions(root)
        self.assertIn(blax.TargetVersion.PY38, versions)

    def test_detect_debug_f_strings(self) -> None:
        root = blax.lib2to3_parse("""f"{x=}" """)
        features = blax.get_features_used(root)
        self.assertIn(blax.Feature.DEBUG_F_STRINGS, features)
        versions = blax.detect_target_versions(root)
        self.assertIn(blax.TargetVersion.PY38, versions)

        root = blax.lib2to3_parse(
            """f"{x}"\nf'{"="}'\nf'{(x:=5)}'\nf'{f(a="3=")}'\nf'{x:=10}'\n"""
        )
        features = blax.get_features_used(root)
        self.assertNotIn(blax.Feature.DEBUG_F_STRINGS, features)
    ...
    def test_string_quotes(self) -> None:
        ...
        mode = blax.Mode(preview=True)
        assert_format(source, expected, mode)
        mode = replace(mode, string_normalization=False)
        not_normalized = fs(source, mode=mode)
        self.assertFormatEqual(source.replace("\\\n", ""), not_normalized)
        blax.assert_equivalent(source, not_normalized)
        blax.assert_stable(source, not_normalized, mode=mode)
    ...
    def test_skip_source_first_line(self) -> None:
        ...
        result = BlackRunner().invoke(
            blax.main, [str(tmp_file), "-x", f"--config={EMPTY_CONFIG}"]
        )
        self.assertEqual(result.exit_code, 0)
        actual = tmp_file.read_text(encoding="utf-8")
    ...
    def test_skip_source_first_line_when_mixing_newlines(self) -> None:
        ...
        with TemporaryDirectory() as workspace:
            ...
            ff(test_file, mode=mode, write_back=blax.WriteBack.YES)
    ...
    def test_skip_magic_trailing_comma(self) -> None:
        ...
        try:
            result = BlackRunner().invoke(
                blax.main, ["-C", "--diff", str(tmp_file), f"--config={EMPTY_CONFIG}"]
            )
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
    ...
    def test_async_as_identifier(self) -> None:
        ...
        actual = fs(source)
        self.assertFormatEqual(expected, actual, source=source, source_path=source_path)
        major, minor = sys.version_info[:2]
        if major < 3 or (major <= 3 and minor < 7):
            blax.assert_equivalent(source, actual)
        blax.assert_stable(source, actual, DEFAULT_MODE)
        # ensure blax can parse this when the target is 3.6
    ...
    def test_python37(self) -> None:
        ...
        if major > 3 or (major == 3 and minor >= 7):
            blax.assert_equivalent(source, actual)
        blax.assert_stable(source, actual, DEFAULT_MODE)
        # ensure blax can parse this when the target is 3.7
    ...
    def test_false_positive_symlink_output_issue_3384(self) -> None:
        # Emulate the behavior when using the CLI (`blax ./child  --verbose`), which
    ...
    def test_report_verbose(self) -> None:
        ...
        with patch("blax.output._out", out), patch("blax.output._err", err):
            report.done(Path("f1"), blax.Changed.NO)
            self.assertEqual(len(out_lines), 1)
            self.assertEqual(len(err_lines), 0)
            self.assertEqual(out_lines[-1], "f1 already well formatted, good job.")
            self.assertEqual(unstyle(str(report)), "1 file left unchanged.")
            self.assertEqual(report.return_code, 0)
            report.done(Path("f2"), blax.Changed.YES)
            ...
            report.done(Path("f3"), blax.Changed.CACHED)
            ...
            report.done(Path("f3"), blax.Changed.YES)
            ...
            self.assertEqual(report.return_code, 123)
            report.done(Path("f4"), blax.Changed.NO)
    ...
    def test_lib2to3_parse(self) -> None:
        with self.assertRaises(blax.InvalidInput):
            blax.lib2to3_parse("invalid syntax")

        straddling = "x + y"
        blax.lib2to3_parse(straddling)
        blax.lib2to3_parse(straddling, {TargetVersion.PY36})

        py2_only = "print x"
        with self.assertRaises(blax.InvalidInput):
            blax.lib2to3_parse(py2_only, {TargetVersion.PY36})

        py3_only = "exec(x, end=y)"
        blax.lib2to3_parse(py3_only)
        blax.lib2to3_parse(py3_only, {TargetVersion.PY36})
    ...
    def test_get_features_used_decorator(self) -> None:
        ...
        for simple_test in simples.split("##")[1:]:
            node = blax.lib2to3_parse(simple_test)
            decorator = str(node.children[0].children[0]).strip()
            self.assertNotIn(
                Feature.RELAXED_DECORATORS,
                blax.get_features_used(node),
                msg=(
        ...
        for relaxed_test in relaxed.split("##")[1:]:
            node = blax.lib2to3_parse(relaxed_test)
            decorator = str(node.children[0].children[0]).strip()
            self.assertIn(
                Feature.RELAXED_DECORATORS,
                blax.get_features_used(node),
    ...
    def check_features_used(self, source: str, expected: Set[Feature]) -> None:
        node = blax.lib2to3_parse(source)
        actual = blax.get_features_used(node)
    ...
    def test_get_features_used_for_future_flags(self) -> None:
        for src, features in [
            ...
            with self.subTest(src=src, features=features):
                node = blax.lib2to3_parse(src)
                future_imports = blax.get_future_imports(node)
                self.assertEqual(
                    blax.get_features_used(node, future_imports=future_imports),
                    features,
                )

    def test_get_future_imports(self) -> None:
        node = blax.lib2to3_parse("\n")
        self.assertEqual(set(), blax.get_future_imports(node))
        node = blax.lib2to3_parse("from __future__ import blax\n")
        self.assertEqual({"blax"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse("from __future__ import multiple, imports\n")
        self.assertEqual({"multiple", "imports"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse("from __future__ import (parenthesized, imports)\n")
        self.assertEqual({"parenthesized", "imports"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse(
            "from __future__ import multiple\nfrom __future__ import imports\n"
        )
        self.assertEqual({"multiple", "imports"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse("# comment\nfrom __future__ import blax\n")
        self.assertEqual({"blax"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse('"""docstring"""\nfrom __future__ import blax\n')
        self.assertEqual({"blax"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse("some(other, code)\nfrom __future__ import blax\n")
        self.assertEqual(set(), blax.get_future_imports(node))
        node = blax.lib2to3_parse("from some.module import blax\n")
        self.assertEqual(set(), blax.get_future_imports(node))
        node = blax.lib2to3_parse(
            "from __future__ import unicode_literals as _unicode_literals"
        )
        self.assertEqual({"unicode_literals"}, blax.get_future_imports(node))
        node = blax.lib2to3_parse(
            "from __future__ import unicode_literals as _lol, print"
        )
        self.assertEqual({"unicode_literals", "print"}, blax.get_future_imports(node))
    ...
    def test_debug_visitor(self) -> None:
        ...
        with patch("blax.debug.out", out):
            DebugVisitor.show(source)
        actual = "\n".join(out_lines) + "\n"
        log_name = ""
        if expected != actual:
            log_name = blax.dump_to_file(*out_lines)
        self.assertEqual(
            expected,
            actual,
    ...
    def test_format_file_contents(self) -> None:
        mode = DEFAULT_MODE
        empty = ""
        with self.assertRaises(blax.NothingChanged):
            blax.format_file_contents(empty, mode=mode, fast=False)
        just_nl = "\n"
        with self.assertRaises(blax.NothingChanged):
            blax.format_file_contents(just_nl, mode=mode, fast=False)
        same = "j = [1, 2, 3]\n"
        with self.assertRaises(blax.NothingChanged):
            blax.format_file_contents(same, mode=mode, fast=False)
        different = "j = [1,2,3]"
        expected = same
        actual = blax.format_file_contents(different, mode=mode, fast=False)
        self.assertEqual(expected, actual)
        invalid = "return if you can"
        with self.assertRaises(blax.InvalidInput) as e:
            blax.format_file_contents(invalid, mode=mode, fast=False)
        self.assertEqual(str(e.exception), "Cannot parse: 1:7: return if you can")

        just_crlf = "\r\n"
        with self.assertRaises(blax.NothingChanged):
            blax.format_file_contents(just_crlf, mode=mode, fast=False)
        just_whitespace_nl = "\n\t\n \n\t \n \t\n\n"
        actual = blax.format_file_contents(just_whitespace_nl, mode=mode, fast=False)
        self.assertEqual("\n", actual)
        just_whitespace_crlf = "\r\n\t\r\n \r\n\t \r\n \t\r\n\r\n"
        actual = blax.format_file_contents(just_whitespace_crlf, mode=mode, fast=False)
        self.assertEqual("\r\n", actual)

    def test_endmarker(self) -> None:
        n = blax.lib2to3_parse("\n")
        self.assertEqual(n.type, blax.syms.file_input)
        self.assertEqual(len(n.children), 1)
        self.assertEqual(n.children[0].type, blax.token.ENDMARKER)
    ...
    def test_single_file_force_pyi(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            pyi_cache = blax.Cache.read(pyi_mode)
            assert not pyi_cache.is_changed(path)
            normal_cache = blax.Cache.read(DEFAULT_MODE)
            assert normal_cache.is_changed(path)
        self.assertFormatEqual(expected, actual)
        blax.assert_equivalent(contents, actual)
        blax.assert_stable(contents, actual, pyi_mode)
    ...
    def test_multi_file_force_pyi(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            # verify cache with --pyi is separate
            pyi_cache = blax.Cache.read(pyi_mode)
            normal_cache = blax.Cache.read(reg_mode)
            for path in paths:
                assert not pyi_cache.is_changed(path)
                assert normal_cache.is_changed(path)
    ...
    def test_pipe_force_pyi(self) -> None:
        ...
        result = CliRunner().invoke(
            blax.main, ["-", "-q", "--pyi"], input=BytesIO(source.encode("utf-8"))
        )
        self.assertEqual(result.exit_code, 0)
        actual = result.output
    ...
    def test_single_file_force_py36(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            py36_cache = blax.Cache.read(py36_mode)
    ...
    def test_multi_file_force_py36(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            for path in paths:
                ...
                self.assertEqual(actual, expected)
            # verify cache with --target-version is separate
            pyi_cache = blax.Cache.read(py36_mode)
            normal_cache = blax.Cache.read(reg_mode)
            for path in paths:
                assert not pyi_cache.is_changed(path)
                assert normal_cache.is_changed(path)
    ...
    def test_reformat_one_with_stdin(self) -> None:
        with patch(
            "blax.format_stdin_to_stdout",
            return_value=lambda *args, **kwargs: blax.Changed.YES,
        ) as fsts:
            report = MagicMock()
            path = Path("-")
            blax.reformat_one(
                path,
                fast=True,
                write_back=blax.WriteBack.YES,
                mode=DEFAULT_MODE,
                report=report,
            )
            fsts.assert_called_once()
            report.done.assert_called_with(path, blax.Changed.YES)
    ...
    def test_reformat_one_with_stdin_filename(self) -> None:
        with patch(
            ...
            fsts.assert_called_once_with(
                fast=True, write_back=blax.WriteBack.YES, mode=DEFAULT_MODE, lines=()
            )
            # __BLACK_STDIN_FILENAME__ should have been stripped
            report.done.assert_called_with(expected, blax.Changed.YES)
    ...
    def test_reformat_one_with_stdin_and_existing_path(self) -> None:
        with patch(
            ...
            report = MagicMock()
            # Even with an existing file, since we are forcing stdin, blax
    ...
    def test_reformat_one_with_stdin_empty(self) -> None:
        ...
        def _new_wrapper(
            ...
            def get_output(*args: Any, **kwargs: Any) -> io.TextIOWrapper:
                ...
                # See discussion in https://github.com/psf/blax/pull/2489
    ...
    def test_required_version_matches_version(self) -> None:
        self.invokeBlack(
            ["--required-version", blax.__version__, "-c", "0"],
            exit_code=0,
            ignore_config=True,
        )
    ...
    def test_required_version_matches_partial_version(self) -> None:
        self.invokeBlack(
            ["--required-version", blax.__version__.split(".")[0], "-c", "0"],
            exit_code=0,
            ignore_config=True,
        )
    ...
    def test_required_version_does_not_match_on_minor_version(self) -> None:
        self.invokeBlack(
            ["--required-version", blax.__version__.split(".")[0] + ".999", "-c", "0"],
            exit_code=1,
            ignore_config=True,
        )
    ...
    def test_preserves_line_endings(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            for nl in ["\n", "\r\n"]:
                ...
                ff(test_file, write_back=blax.WriteBack.YES)
    ...
    def test_preserves_line_endings_via_stdin(self) -> None:
        for nl in ["\n", "\r\n"]:
            ...
            result = runner.invoke(
                blax.main, ["-", "--fast"], input=BytesIO(contents.encode("utf-8"))
            )
            self.assertEqual(result.exit_code, 0)
    ...
    def test_invalid_config_return_code(self) -> None:
        tmp_file = Path(blax.dump_to_file())
        try:
            tmp_config = Path(blax.dump_to_file())
            tmp_config.unlink()
    ...
    def test_parse_pyproject_toml(self) -> None:
        ...
        config = blax.parse_pyproject_toml(str(test_toml_file))
    ...
    def test_infer_target_version(self) -> None:
        for version, expected in [
            ...
            result = blax.files.infer_target_version(test_toml)
            self.assertEqual(result, expected)

    def test_read_pyproject_toml(self) -> None:
        test_toml_file = THIS_DIR / "test.toml"
        fake_ctx = FakeContext()
        blax.read_pyproject_toml(fake_ctx, FakeParameter(), str(test_toml_file))
    ...
    def test_read_pyproject_toml_from_stdin(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            with change_directory(root):
                blax.read_pyproject_toml(fake_ctx, FakeParameter(), None)
    ...
    def test_find_project_root(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            root_pyproject.write_text("[tool.blax]", encoding="utf-8")
            src_pyproject = src_dir / "pyproject.toml"
            src_pyproject.write_text("[tool.blax]", encoding="utf-8")
            src_python = src_dir / "foo.py"
            src_python.touch()

            self.assertEqual(
                blax.find_project_root((src_dir, test_dir)),
                (root.resolve(), "pyproject.toml"),
            )
            self.assertEqual(
                blax.find_project_root((src_dir,)),
                (src_dir.resolve(), "pyproject.toml"),
            )
            self.assertEqual(
                blax.find_project_root((src_python,)),
                (src_dir.resolve(), "pyproject.toml"),
            )

            with change_directory(test_dir):
                self.assertEqual(
                    blax.find_project_root(("-",), stdin_filename="../src/a.py"),
                    (src_dir.resolve(), "pyproject.toml"),
                )
            ...
            # we skip src_sub_pyproject since it is missing the [tool.blax] section
            self.assertEqual(
                blax.find_project_root((src_sub_python,)),
                (src_dir.resolve(), "pyproject.toml"),
            )

    @patch(
        "blax.files.find_user_pyproject_toml",
    )
    def test_find_pyproject_toml(self, find_user_pyproject_toml: MagicMock) -> None:
        ...
        with redirect_stderr(io.StringIO()) as stderr:
            result = blax.files.find_pyproject_toml(
    ...
    @patch(
        "blax.files.find_user_pyproject_toml",
        blax.files.find_user_pyproject_toml.__wrapped__,
    )
    def test_find_user_pyproject_toml_linux(self) -> None:
        ...
        with TemporaryDirectory() as workspace:
            tmp_user_config = Path(workspace) / "blax"
            with patch.dict("os.environ", {"XDG_CONFIG_HOME": workspace}):
                self.assertEqual(
                    blax.files.find_user_pyproject_toml(), tmp_user_config.resolve()
                )

        # Test fallback for XDG_CONFIG_HOME
        with patch.dict("os.environ"):
            os.environ.pop("XDG_CONFIG_HOME", None)
            fallback_user_config = Path("~/.config").expanduser() / "blax"
            self.assertEqual(
                blax.files.find_user_pyproject_toml(), fallback_user_config.resolve()
            )
    ...
    def test_find_user_pyproject_toml_windows(self) -> None:
        ...
        user_config_path = Path.home() / ".blax"
        self.assertEqual(
            blax.files.find_user_pyproject_toml(), user_config_path.resolve()
        )
    ...
    def test_bpo_33660_workaround(self) -> None:
        ...
        with change_directory(root):
            ...
            report = blax.Report(verbose=True)
            resolves_outside = blax.resolves_outside_root_or_cannot_stat(
                path, root, report
            )
    ...
    def test_newline_comment_interaction(self) -> None:
        source = "class A:\\\r\n# type: ignore\n pass\n"
        output = blax.format_str(source, mode=DEFAULT_MODE)
        blax.assert_stable(source, output, mode=DEFAULT_MODE)
    ...
    def test_bpo_2142_workaround(self) -> None:
        ...
        tmp_file = Path(blax.dump_to_file(source, ensure_final_newline=False))
        diff_header = re.compile(
            rf"{re.escape(str(tmp_file))}\t\d\d\d\d-\d\d-\d\d "
            r"\d\d:\d\d:\d\d\.\d\d\d\d\d\d\+\d\d:\d\d"
        )
        try:
            result = BlackRunner().invoke(blax.main, ["--diff", str(tmp_file)])
            self.assertEqual(result.exit_code, 0)
        finally:
            os.unlink(tmp_file)
        actual = result.output
    ...
    def test_code_option(self) -> None:
        ...
        code = 'print("Hello world")\n'
        args = ["--code", code]
        result = CliRunner().invoke(blax.main, args)
    ...
    def test_code_option_changed(self) -> None:
        ...
        code = "print('hello world')"
        formatted = blax.format_str(code, mode=DEFAULT_MODE)
    ...
    def test_code_option_safe(self) -> None:
        ...
        # Patch blax.assert_equivalent to ensure the sanity checks fail
        with patch.object(blax, "assert_equivalent", side_effect=AssertionError):
            code = 'print("Hello world")'
    ...
    def test_code_option_config(self) -> None:
        ...
        """
        with patch.object(blax, "parse_pyproject_toml", return_value={}) as parse:
            args = ["--code", "print"]
            # This is the only directory known to contain a pyproject.toml
            with change_directory(PROJECT_ROOT):
                CliRunner().invoke(blax.main, args)
    ...
    def test_for_handled_unexpected_eof_error(self) -> None:
        ...
        """
        with pytest.raises(blax.parsing.InvalidInput) as exc_info:
            blax.lib2to3_parse("print(", {})
    ...
    def test_line_ranges_with_stdin(self) -> None:
        ...
        runner = BlackRunner()
        result = runner.invoke(
            blax.main, ["--line-ranges=1-1", "-"], input=BytesIO(code.encode("utf-8"))
        )
    ...
    def test_line_ranges_in_pyproject_toml(self) -> None:
        ...
        result = BlackRunner().invoke(
            blax.main, ["--code", "print()", "--config", str(config)]
        )
...
class TestCaching:
    def test_get_cache_dir(
        ...
        patch_user_cache_dir = patch(
            target="blax.cache.user_cache_dir",
            autospec=True,
    ...
    def test_cache_broken_file(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            assert blax.Cache.read(mode).file_data == {}
            src = (workspace / "test.py").resolve()
            src.write_text("print('hello')", encoding="utf-8")
            invokeBlack([str(src)])
            cache = blax.Cache.read(mode)
    ...
    def test_no_cache_when_writeback_diff(self, color: bool) -> None:
        ...
        with cache_dir() as workspace:
            src = (workspace / "test.py").resolve()
            src.write_text("print('hello')", encoding="utf-8")
            with patch.object(blax.Cache, "read") as read_cache, patch.object(
                blax.Cache, "write"
            ) as write_cache:
                cmd = [str(src), "--diff"]
                if color:
    ...
    def test_output_locking_when_writeback_diff(self, color: bool) -> None:
        with cache_dir() as workspace:
            for tag in range(0, 4):
                ...
                src.write_text("print('hello')", encoding="utf-8")
            with patch(
                "blax.concurrency.Manager", wraps=multiprocessing.Manager
            ) as mgr:
    ...
    def test_no_cache_when_stdin(self) -> None:
        ...
        with cache_dir():
            result = CliRunner().invoke(
                blax.main, ["-"], input=BytesIO(b"print('hello')")
            )
            assert not result.exit_code
            cache_file = get_cache_file(mode)
    ...
    def test_write_cache_read_cache(self) -> None:
        ...
        with cache_dir() as workspace:
            src = (workspace / "test.py").resolve()
            src.touch()
            write_cache = blax.Cache.read(mode)
            write_cache.write([src])
            read_cache = blax.Cache.read(mode)
    ...
    def test_filter_cached(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            cache = blax.Cache.read(DEFAULT_MODE)

            orig_func = blax.Cache.get_file_data
            ...
            with patch.object(blax.Cache, "get_file_data", side_effect=wrapped_func):
    ...
    def test_filter_cached_hash(self) -> None:
        with TemporaryDirectory() as workspace:
            ...
            assert cached_file_data.hash == blax.Cache.hash_digest(src)
    ...
    def test_read_cache_line_lengths(self) -> None:
        ...
        with cache_dir() as workspace:
            ...
            cache = blax.Cache.read(mode)
            cache.write([path])
            one = blax.Cache.read(mode)
            assert not one.is_changed(path)
            two = blax.Cache.read(short_mode)
...
class TestFileCollection:
    ...
    @patch("blax.find_project_root", lambda *args: (THIS_DIR.resolve(), None))
    def test_exclude_for_issue_1572(self) -> None:
        # Exclude shouldn't touch files that were explicitly given to Black through the
        # CLI. Exclude is supposed to only apply to the recursive discovery of files.
        # https://github.com/psf/blax/issues/1572
    ...
    def test_nested_gitignore(self) -> None:
        ...
        include = re.compile(r"\.pyi?$")
        exclude = re.compile(r"")
        root_gitignore = blax.files.get_gitignore(path)
        report = blax.Report()
        expected: List[Path] = [
            Path(path / "x.py"),
            Path(path / "root/b.py"),
    ...
    def test_nested_gitignore_directly_in_source_directory(self) -> None:
        # https://github.com/psf/blax/issues/2598
    ...
    def test_invalid_gitignore(self) -> None:
        ...
        result = BlackRunner().invoke(
            blax.main, ["--verbose", "--config", str(empty_config), str(path)]
        )
        assert result.exit_code == 1
        assert result.stderr_bytes is not None
    ...
    def test_symlinks(self) -> None:
        ...
        include = re.compile(blax.DEFAULT_INCLUDES)
        exclude = re.compile(blax.DEFAULT_EXCLUDES)
        report = blax.Report()
    ...
    def test_get_sources_with_stdin_symlink_outside_root(
        ...
        with TemporaryDirectory() as tempdir:
            ...
            root = tmp / "root"
            root.mkdir()
            (root / "pyproject.toml").write_text("[tool.blax]", encoding="utf-8")
...
class TestDeFactoAPI:
    ...
    def test_format_str(self) -> None:
        ...
        assert (
            blax.format_str("print('hello')", mode=blax.Mode()) == 'print("hello")\n'
        )

        # you can pass line length
        assert (
            blax.format_str("print('hello')", mode=blax.Mode(line_length=42))
            == 'print("hello")\n'
        )

        # invalid input raises InvalidInput
        with pytest.raises(blax.InvalidInput):
            blax.format_str("syntax error", mode=blax.Mode())

    def test_format_file_contents(self) -> None:
        # You probably should be using format_str() instead, but let's keep
        # this one around since people do use it
        assert (
            blax.format_file_contents("x=1", fast=True, mode=blax.Mode()) == "x = 1\n"
        )

        with pytest.raises(blax.NothingChanged):
            blax.format_file_contents("x = 1\n", fast=True, mode=blax.Mode())
...
class TestASTSafety(BlackBaseTestCase):
    def check_ast_equivalence(
        ...
        blax.parse_ast(source)
        blax.parse_ast(dest)
        if should_fail:
            with self.assertRaises(ASTSafetyError):
                blax.assert_equivalent(source, dest)
        else:
            blax.assert_equivalent(source, dest)
    ...
    def test_equivalency_ast_parse_failure_includes_error(self) -> None:
        with pytest.raises(ASTSafetyError) as err:
            blax.assert_equivalent("a«»a  = 1", "a«»a  = 1")
...
try:
    with open(blax.__file__, "r", encoding="utf-8") as _bf:
        black_source_lines = _bf.readlines()
except UnicodeDecodeError:
    if not blax.COMPILED:
        raise
...
def tracefunc(
    ...
    """Show function calls `from blax/__init__.py` as they happen.
    ...
    while funcname.startswith("@"):
        func_sig_lineno += 1
        funcname = black_source_lines[func_sig_lineno].strip()
    if "blax/__init__.py" in filename:
