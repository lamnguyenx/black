...
class Line:
    ...
    def get_all_properties(self) -> dict:
        return {
            ...
            'is_triple_quoted_string'    : self._is_triple_quoted_string,
...
class EmptyLineTracker:
    ...
    def maybe_empty_lines(self, current_line: Line) -> LinesBlock:
        ...
        # unfold = nice_utils.nested_object_unfold
        # logger.green('###################################')
        form_feed = (
        ...
        # current_line_unfolded = \
        # nice_utils.nested_json_dict_search_then_set(
        #     nice_utils.unfold(current_line),
        #     matching_func=lambda k,V : k in ('leaves', 'invisible'),
        #     setting_func=lambda k,V : (k, '<DELETED>'),
        # )[0]
        # logger.green('leaves', ''.join(map(str, current_line.leaves)))
        # logger.green('current_line_unfolded', current_line_unfolded)
        # logger.green('before',before)
        # logger.green('after',after)
    ...
    def _maybe_empty_lines(self, current_line: Line) -> Tuple[int, int]:  # noqa: C901
        ...
        if current_line.depth == 0:
            max_allowed = 1 if self.mode.is_pyi else self.mode.STYLE.no_empty_lines.default
        ...
        if previous_def is not None:
            ...
            if self.mode.is_pyi:
                ...
                if depth:
                    ...
                    before = self.mode.STYLE.no_empty_lines.default
        ...
        if (
            ...
            if self.mode.STYLE.__name__ == 'STYLE_LAMNGUYENX':
        ...
        if (
            ...
            and current_line._is_triple_quoted_string
        ):
        ...
        is_empty_first_line_ok = (
            ...
            and (
                not is_docstring(current_line.leaves[0], mode=self.mode)
                or (
                    self.previous_line
        ...
        if (
            ...
            if self.mode.STYLE.__name__ == 'STYLE_LAMNGUYENX':
                if (self.previous_line.is_class):
                    return self.mode.STYLE.no_empty_lines.opens_block, 0
    ...
    def _maybe_empty_lines_for_class_or_def(  # noqa: C901
        ...
        if self.previous_line is None:
            # Don't insert empty lines before the first line in the file.
            return self.mode.STYLE.no_empty_lines.between_class_and_first_one, 0
        ...
        if self.mode.is_pyi:
            ...
            newlines = self.mode.STYLE.no_empty_lines.between_class_methods if current_line.depth else self.mode.STYLE.no_empty_lines.default
