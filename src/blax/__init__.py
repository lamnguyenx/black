# import nice_utils; logger = nice_utils.getLogger(__name__)
# hu = nice_utils
# import inspect
...
try:
    ...
    from ._black_version_fake import version as __version__
...
from .linegen import LN, LineGenerator, transform_line
from .lines import EmptyLineTracker, LinesBlock, Line
from .mode import FUTURE_FLAG_TO_FEATURE, VERSION_TO_FEATURES, Feature
from .mode import Mode as Mode  # re-exported
from .mode import Preview, TargetVersion, supports_feature
from .nodes import (
    STARS,
    is_number_token,
...
from .output import color_diff, diff, dump_to_file, err, ipynb_diff, out
from .parsing import InvalidInput  # noqa F401
from .parsing import lib2to3_parse, parse_ast, stringify_ast
from .ranges import adjusted_lines, convert_unchanged_lines, parse_line_ranges
from .report import Changed, NothingChanged, Report
from .trans import iter_fexpr_spans
from .blib2to3.pgen2 import token
from .blib2to3.pytree import Leaf, Node
from .lamnguyenx.format_super_alignment import format_super_alignment
from .lamnguyenx.format_super_brackets import format_super_brackets
from .lamnguyenx.format_super_empty_lines import format_super_empty_lines
from .lamnguyenx.booleanify import booleanify
from .lamnguyenx.nested_show_level import nested_node_show_level
from .lamnguyenx.jot_line import jot_line
...
@click.option(
    "-l",
    "--line_length".replace("_", "-"),
    type=int,
    default=STYLE_DEFAULT.max_line_length,
...
@click.option(
    "-S",
    "--skip_string_normalization".replace("_", "-"),
    is_flag=True,
...
@click.option(
    "--run_as_black".replace("_", "-"),
    type=booleanify,
    default=os.environ.get("RUN_AS_BLACK", False),
    show_default=True,
)
@click.pass_context
def main(  # noqa: C901
    ...
    if not run_as_black:
        ...
        verbose = True
        line_length = STYLE_LAMNGUYENX.max_line_length
    ...
    mode = Mode(
        ...
        STYLE=STYLE_DEFAULT if run_as_black else STYLE_LAMNGUYENX,
    )
    ...
    if code is not None:
        ...
        if len(sources) == 1:
            ...
            from .concurrency import reformat_many
...
def _format_str_once(
    ...
    # jot_line(src_node, 'src_note', 0)

    for i, current_line in enumerate(line_generator.visit(src_node)):
        # jot_line(current_line, 'current_line', i)
        ...
        for j, line in enumerate(
            ...
            # jot_line(line, 'line', i*100 + j)

            if mode.STYLE.__name__ == "STYLE_LAMNGUYENX":
                # added vertical spacing to multiple imports
    ...
    #     if (block.Content_Lines is not None) and (len(block.Content_Lines) > 0):
    #         block.Content_Lines[0].before = block.before
    #         First_Block_Lines.append(block.Content_Lines[0])

    if mode.STYLE.__name__ == "STYLE_LAMNGUYENX":
        format_super_alignment(dst_blocks, mode=mode)
        # format_super_brackets(dst_blocks, mode=mode)
